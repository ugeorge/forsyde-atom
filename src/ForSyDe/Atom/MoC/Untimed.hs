...
module ForSyDe.Atom.MoC.Untimed where
...
class MoC e where
  ...
  -- of arguments.
  (-$-)  :: (Nat c) => (Value (FSVec c a) -> b)
            -> Signal (e (Value a)) -> Signal (e b)
  ...
  -- composition of generic process constructors with arbitrary number
  -- of arguments.
  (-*-) :: (Nat c) => Signal (e (Value (FSVec c a) -> b))
           -> Signal (e (Value a)) -> Signal (e b)

  post :: (Nat p) => Signal (e (Value (FSVec p a))) -> Signal (e (Value a))
  ...
  (-&>-) :: (Nat n) => e (Value (FSVec n a)) -> Signal (e (Value a)) -> Signal (e (Value a))
   

           
comb11 f s1             = (post)                $    (f -$- s1)
comb12 f s1             = (post,post)           $$   (f -$- s1 ||<)
comb13 f s1             = (post,post,post)      $$$  (f -$- s1 ||<<)
comb14 f s1             = (post,post,post,post) $$$$ (f -$- s1 ||<<<)
comb21 f s1 s2          = (post)                $    (f -$- s1 -*- s2)
comb22 f s1 s2          = (post,post)           $$   (f -$- s1 -*- s2 ||<)
comb23 f s1 s2          = (post,post,post)      $$$  (f -$- s1 -*- s2 ||<<)
comb24 f s1 s2          = (post,post,post,post) $$$$ (f -$- s1 -*- s2 ||<<<)
comb31 f s1 s2 s3       = (post)                $    (f -$- s1 -*- s2 -*- s3)
comb32 f s1 s2 s3       = (post,post)           $$   (f -$- s1 -*- s2 -*- s3 ||<)
comb33 f s1 s2 s3       = (post,post,post)      $$$  (f -$- s1 -*- s2 -*- s3 ||<<)
comb34 f s1 s2 s3       = (post,post,post,post) $$$$ (f -$- s1 -*- s2 -*- s3 ||<<<)
comb41 f s1 s2 s3 s4    = (post)                $    (f -$- s1 -*- s2 -*- s3 -*- s4)
comb42 f s1 s2 s3 s4    = (post,post)           $$   (f -$- s1 -*- s2 -*- s3 -*- s4 ||<)
comb43 f s1 s2 s3 s4    = (post,post,post)      $$$  (f -$- s1 -*- s2 -*- s3 -*- s4 ||<<)
comb44 f s1 s2 s3 s4    = (post,post,post,post) $$$$ (f -$- s1 -*- s2 -*- s3 -*- s4 ||<<<)
comb51 f s1 s2 s3 s4 s5 = (post)                $    (f -$- s1 -*- s2 -*- s3 -*- s4 -*- s5)
comb52 f s1 s2 s3 s4 s5 = (post,post)           $$   (f -$- s1 -*- s2 -*- s3 -*- s4 -*- s5 ||<)
comb53 f s1 s2 s3 s4 s5 = (post,post,post)      $$$  (f -$- s1 -*- s2 -*- s3 -*- s4 -*- s5 ||<<)
comb54 f s1 s2 s3 s4 s5 = (post,post,post,post) $$$$ (f -$- s1 -*- s2 -*- s3 -*- s4 -*- s5 ||<<<)

delay i xs = i -&>- xs

scanl11 ns i s1          = comb21 ns st s1 
  where st               = delay             i $  comb21 ns st s1 
scanl12 ns i s1          = (comb32 ns <> st) s1
  where st               = (delay, delay) $$ i $$ scanl12 ns i s1 
...
scanld02 ns i             = st
  where st                = (delay, delay) $$ i $$ (comb22 ns <> st)
scanld03 ns i             = (st |||<<)
  where st                = i -&>- comb11 ns st
scanld04 ns i             = (st |||<<<)
  where st                = i -&>- comb11 ns st
